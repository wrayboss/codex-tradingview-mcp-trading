...
function pushStrategyTesterMetricBlocker(metricBlockers, {
  metric,
  gate,
  label,
  actual,
...
export function buildStrategyTesterMetricBlockers(summary = {}, gates = DEFAULT_GATES) {
  ...
  const winRate = parseVisibleStrategyTesterPercentRatio(metrics.winRate ?? metrics.profitableTrades);
  if (winRate !== null && winRate < gates.minWinRate) {
    pushStrategyTesterMetricBlocker(metricBlockers, {
      metric: "winRate",
      gate: 2,
      label: "Win rate",
      actual: winRate,
      threshold: gates.minWinRate,
      comparator: ">=",
      detail: `Win rate ${formatVisibleStrategyTesterPercent(winRate)} is below approval threshold ${formatVisibleStrategyTesterPercent(gates.minWinRate)}.`,
    });
  }
  ...
  if (maxDrawdown !== null && maxDrawdown > gates.maxDrawdownPct) {
    pushStrategyTesterMetricBlocker(metricBlockers, {
      ...
      label: "Max drawdown",
      actual: maxDrawdown,
      threshold: gates.maxDrawdownPct,
      comparator: "<=",
      detail: `Max drawdown ${formatVisibleStrategyTesterPercent(maxDrawdown)} is above approval threshold ${formatVisibleStrategyTesterPercent(gates.maxDrawdownPct)}.`,
    });
  }

  const totalTrades = parseVisibleStrategyTesterNumber(metrics.totalTrades);
  if (totalTrades !== null && totalTrades < gates.minTradesPerSymbol) {
    pushStrategyTesterMetricBlocker(metricBlockers, {
      metric: "totalTrades",
      gate: 5,
      label: "Trade count",
      actual: totalTrades,
      threshold: gates.minTradesPerSymbol,
      comparator: ">=",
      detail: `Trade count ${totalTrades} is below approval threshold ${gates.minTradesPerSymbol}.`,
    });
  }
